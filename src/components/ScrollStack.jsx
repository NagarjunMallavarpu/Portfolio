...
const ScrollStack = ({
  ...
  const calculateCardOffsets = useCallback(() => {
    ...
    // Helper to get absolute offset from the top of the document
    ...
    if (endElement) {
      endElementOffsetRef.current = getAbsoluteTop(endElement);
    }
  }, []);
  ...
  useLayoutEffect(() => {
    ...
    return () => {
      ...
      if (lenisRef.current) {
        if (useWindowScroll && window.lenis && lenisRef.current === window.lenis) {
          window.lenis.off('scroll', handleScroll);
        } else if (typeof lenisRef.current.destroy === 'function') {
